...
def todict(obj, classkey=None, level=0):
    logger.info("Object %s", type(obj))
    if isinstance(obj, dict):
        logger.info("isinstance dict")
        data = {}
...
def process():
    ...
    model = data.get("model") or "en_core_web_sm"
    if MODELS.get(model) is None:
        logger.info("Loading model %s", model)
        MODELS[model] = spacy.load(model)
